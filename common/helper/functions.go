...
func Log(txt ...interface{}) {
	if FrameInfo["env"] == "debug" || FrameInfo["env"] == "dev" {
		log.Println("ShowLog：", txt)
	}
}
func Error(txt ...interface{}) {
	if FrameInfo["env"] == "debug" || FrameInfo["env"] == "dev"  {
		log.Println("ShowError：", txt)
	}
}
...
// SaltPwdAdmin 防护
func SaltPwdAdmin(Salt string) string {
	return MD5(Salt+"gV22php")
}

// SaltPwdUser 防护
func SaltPwdUser(Salt string) string {
	return MD5(Salt+"Gv2g0")
}
...
// StringToArray 字符串转一维数组，仅支持数字、字母、字符、中文
func StringToArray(_string string, _break string) []string {
	arr := strings.Split(_string, _break)
	//for eq, value := range arr{ // 遍历
	//	fmt.Println(eq, "===>", string(value))
	//}
	return arr
}
...
func MakePaging(total int64, limit int64, page int64) map[string]interface{}{
	_total := int(total)
	_limit := int(limit)
	_page := int(page)
	// 总页数
	...
	var back = map[string]interface{}{
		...
		"calc": map[string]interface{}{
			"total_page":  int64(pageTotal),
			"first_page":  int64(firstPage),
			"last_page":   int64(lastPage),
			"before_page": int64(beforePage),
			"now_page": int64(nowPage),
			"after_page":  int64(afterPage),
			"foot_page": footPage,
		},
	}
	return back
...
// 如隐藏手机号：185*******6，调用helper.HideStringValue("18512345506", 3, 10, "*")
...
func ServerIPv4() []string {
	...
	if len(ipArray) == 0 { // 为空时就默认值
		ipArray = append(ipArray, "0.0.0.0")
	}
