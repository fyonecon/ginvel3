...
func CreateRedisMQ(ctx *gin.Context, key string, id1 int64, id2 int64, callFunc CallFunc) int64 {
	...
	v, _ := json.Marshal(_v)
	err := providers.RedisDb.LPush(ctx, k, v).Err()
	if err != nil {
		fmt.Println(err)
		kits.Log("CreateRedisMQ缓存写入失败", "")
		return 0
	}
	return 1
}
...
func DoRedisMQ(ctx *gin.Context) {
	...
	_k := "gl_cache_mq"
	// 获取表中元素的个数
	_len, _ := providers.RedisDb.LLen(ctx, _k).Result()
	_val, _err := providers.RedisDb.LRange(ctx, _k, 0, _len-1).Result()
	if _err != nil {
		fmt.Println(_err)
		return
	}
	...
	for i := 0; i < int(_len); i++ {
		val := _val[1]
		back := helper.JsonStringToMap(val)
		_key := back["key"]
		_id1 := back["id1"]
		_id2 := back["id2"]
		_callFunc := back["callFunc"]

		key := helper.ValueInterfaceToString(_key)
		id1 := helper.ValueInterfaceToInt(_id1)
		id2 := helper.ValueInterfaceToInt(_id2)
		callFunc := helper.ValueInterfaceToString(_callFunc)
