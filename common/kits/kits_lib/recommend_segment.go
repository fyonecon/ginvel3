...
func MakeSeg(_text string) []string {
	// 只需要纯文本
	_text = helper.FilterXML(_text) // 过滤xml
	_text = helper.FilterHTML(_text) // 过滤html

	text := []byte(_text)

	// 分词插件1（最短分词）
	//var seg sego.Segmenter
	//seg.LoadDictionary(helper.FrameInfo["storage_path"] + "sys_file/seg/data/dictionary.txt")
	//segments := seg.Segment(text)
	//wordSeg := sego.SegmentsToString(segments, false) // 支持普通模式和搜索模式两种分词，false普通模式

	// 分词插件2（最短分词）
	var seg gse.Segmenter
	seg.LoadDict(helper.FrameInfo["storage_path"] + "sys_file/gse/data/dict/zh/dict.txt")
	seg.LoadStop()
	//seg.LoadDictEmbed()
	//seg.LoadStopEmbed()
	...
	wordArray = helper.RemoveRepeatedStringArray(wordArray)
