...
/*
	// 压缩图片示例
	filepath := helper.FrameInfo["storage_path"] + "upload/" // 在服务器里面的绝对路径文件夹
	dateFile := helper.GetTimeDate("Cmp-Ymd") + "/"
	...
	// 压缩图片
	img := kits.CompressImg("color.png", filepath, saveFilepath)
*/
func CompressImg(_imgName string, _filepath string, _saveFilepath string)  (backImgName string) {
	has, _ := helper.HasFile(_filepath+_imgName)
	if has == false {
...
func newImgName() string {
	return fmt.Sprintf(helper.GetTimeDate("YmdHis")+"_"+helper.RandString(helper.RandRange(13, 15))+"_x")
}