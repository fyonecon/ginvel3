...
// HttpLimiterRanking 限制接口请求数据，做到主动降载，超阀值会自动熔断
...
func HttpLimiterRanking(ranking int64) gin.HandlerFunc {
	return func(ctx *gin.Context) {

		// 设置公共参数（设置ctx每次请求的全局值）
		ctx.Set("api_ranking", ranking)
		...
		fmt.Println("RankingLimiter@请求uri=", uri)

		// 读取超全局变量即可
		var _cpuNum interface{} = helper.GetGlobalData("cpu_num")
		var cpuNum int64 = _cpuNum.(int64)
		var _cpuPercent interface{} = helper.GetGlobalData("cpu_percent")
		var __cpuPercent float64 = _cpuPercent.(float64)
		var cpuPercent int64 = int64(math.Floor(__cpuPercent))

		var alertRanking string = "[80, 100]" // 提示优先级范围
		...
		if cpuPercent > ranking { // 直接熔断，等待x秒定时周期后看CPU占用率是否恢复
			kits.Limit("熔断 >>> uri=" + uri, ctx.ClientIP())
			ctx.JSON(429, gin.H{
				"state": 429,
				"msg": "通道拥挤，请稍后再试",
				"content": gin.H{
