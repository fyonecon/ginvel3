...
>介绍：Ginvel是一个MVC+RPC的Golang微服务框架，Http基于Gin，DDD服务基于gRPC。框架需要「 Go1.19^ + MySQL5.7^ + Redis5^ + CPU核心2^ 」的运行环境，开发和部署比较灵活。Ginvel主要用于Web服务后端、电脑客户端软件。
> 
> 特点：Ginvel会利用尽可能多的默认值来代替或减少报错提醒；Ginvel采用"宽进严出"的原则；Ginvel封装了大量数据处理的函数，做到开箱即用，<font color=orange >语意化表达</font>；<font color=orange >编程风格可能更像PHP</font>。
> 
